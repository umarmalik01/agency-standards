...
def mux_audio(video: Path, voice: Path | None, music: Path | None, dst: Path,
    ...
    # apad is load-bearing: the voice track is shorter than the picture (the CTA card carries no
    # narration), and without padding `-shortest` truncates the video to the voice and the CTA
    # never reaches the viewer. Pad the audio out, then let -shortest cut at the video length.
    if voice and music:
        filters.append(f"{inputs[0]}apad[vo]")
        filters.append(f"{inputs[1]}volume={music_gain},apad[bed]")
        filters.append("[vo][bed]amix=inputs=2:duration=longest:dropout_transition=0[aout]")
    else:
        filters.append(f"{inputs[0]}apad[aout]")
